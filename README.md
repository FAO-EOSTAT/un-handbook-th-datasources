## UN Handbook - Chapter 5: Earth observation big data sources

This repository contains the reproducible analysis for Chapter 5 of the UN Handbook on Earth observation big data sources. The project demonstrates how to access and work with Analysis Ready Data (ARD) image collections from various cloud platforms using the `sits` R package and the SpatioTemporal Asset Catalogue (STAC) protocol.
...
```shell
git clone https://github.com/FAO-EOSTAT/un-handbook-th-datasources
```

After cloning, navigate into the project directory using `cd un-handbook-th-datasources`. All subsequent commands should be executed from within this directory.
...
```shell
docker pull faoeostat/handbook:chp5
```
...
```shell
docker build -t "faoeostat/handbook:chp5" --file docker/Dockerfile .
```
...
docker run \
    ...
    --volume ${PWD}:/home/rstudio/handbook faoeostat/handbook:chp5
```
...
Once you have accessed RStudio in your browser, you need to open the project. The project files are available in the `handbook` directory, which is located in the RStudio user home directory. Navigate to this directory using the Files pane in RStudio. Then double-click on the `th_data_sources.Rproj` file to open the project. This will configure RStudio with the correct working directory and project settings.
